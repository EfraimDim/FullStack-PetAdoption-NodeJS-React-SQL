...
function DisplayPet({ pet, myPets, savedPets, allPets }) {
    ...
    try {
      swal({
        ...
        if (willReturn) {
          ...
          await axios.delete(
            `http://localhost:5000/pets/returnForAdoption/${pet.pet_ID}/${pet.name}/${pet.type}/${loggedInInfo.email}`,
            { headers: headers }
          );
    ...
    try {
      swal({
        ...
        if (willAdopt) {
          const headers = await tokenFromLocalforage();
          await axios.put(
            `http://localhost:5000/pets/fosterToAdopt`,
            {
              petID: pet.pet_ID,
              name: pet.name,
              type: pet.type,
  ...
  const unsavePet = async (pet) => {
    try {
      const headers = await tokenFromLocalforage();
      await axios.delete(`http://localhost:5000/pets/unsavePet/${pet.pet_ID}`, { headers: headers });
  ...
  const savePet = async (pet) => {
    try {
      const headers = await tokenFromLocalforage();
      await axios.post(
        `http://localhost:5000/pets/savePet`,
        {
          petID: pet.pet_ID,
        },
        { headers: headers }
    ...
    try {
      swal({
        ...
        if (willAdopt) {
          const headers = await tokenFromLocalforage();
          await axios.post(
            `http://localhost:5000/pets/adoptPet`,
            {
              petID: pet.pet_ID,
              name: pet.name,
              type: pet.type,
    ...
    try {
      swal({
        ...
        if (willFoster) {
          const headers = await tokenFromLocalforage();
          await axios.post(
            `http://localhost:5000/pets/fosterPet`,
            {
              petID: pet.pet_ID,
              name: pet.name,
              type: pet.type,
  ...
  const goToEditPage = (pet) => {
    if (pet.hypoallergenic === 1) pet.hypoallergenic = true;
    else pet.hypoallergenic = false;
    ...
    try {
      swal({
        ...
        if (willDelete) {
          ...
          const headers = await tokenFromLocalforage();
          await axios.delete(
            `http://localhost:5000/pets/deletePet/${pet.pet_ID}/${pet.name}/${pet.type}/${adminInfo.email}/${pet.picture_path}`,
            { headers: headers }
          );
    ...
    <>
      <div id={pet.pet_ID} className={styles.petInfo}>
        ...
        <div className={styles.name}>
          <span className={styles.field}>Name:</span> {pet.name}
        </div>
        <div className={styles.mainInfo}>
          <span className={styles.field}>Status:</span> {pet.adoption_status}
        </div>
        <ShowMore lines={1} more="Show more" less="Show less" anchorClass={styles.showMore}>
          {
            <div className={styles.extraInfoWrapper}>
              <br></br>
              <div className={styles.extraInfo}>
                <div className={styles.info}>
                  <span className={styles.field}>Type:</span> {pet.type}
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Breed:</span> {pet.breed}
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Height:</span> {pet.height}cm
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Weigth:</span> {pet.weight}kg
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Colour:</span> {pet.color}
                </div>
              </div>
              <div>
                <div className={styles.info}>
                  <span className={styles.field}>Bio:</span> {pet.bio}
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Hypoallergenic:</span> {pet.hypoallergenic === 1 ? <span>yes</span> : <span>no</span>}
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Availability:</span> {pet.availability === 1 ? <span>Available</span> : <span>Unavailable</span>}
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Dietry Restrictions:</span> {pet.dietry_restrictions}
                </div>
                <div className={styles.info}>
                  <span className={styles.field}>Date Posted:</span> {pet.date_created.slice(0, 10)}
                </div>
              </div>
            </div>
          }
        </ShowMore>
        {myPets && loggedInInfo && pet.adoption_status === "fostered" && (
          <button className={styles.button} onClick={() => fosterToAdopt(pet)}>
            Adopt
          </button>
        )}

        {loggedInInfo && myPets && (
          <button className={styles.button} onClick={() => returnForAdoption(pet)}>
            Return to Adoption Centre
          </button>
        )}

        {loggedInInfo && savedPets && (
          <button className={styles.button} onClick={() => unsavePet(pet)}>
            Unsave
          </button>
        )}

        {allPets && loggedInInfo && savedPetsArray.filter((savedPet) => savedPet.pet_ID === pet.pet_ID).length === 0 && (
          <button className={styles.button} onClick={() => savePet(pet)}>
            Save For Later
          </button>
        )}
        ...
        {loggedInInfo && allPets && pet.adoption_status === "available" && (
          <>
            <button className={styles.button} onClick={() => adoptPet(pet)}>
              Adopt
            </button>
            <button className={styles.button} onClick={() => fosterPet(pet)}>
              Foster
            </button>
          </>
        )}
        ...
        {adminInfo && (
          <div className={styles.buttonWrapper}>
            <button className={styles.button} onClick={() => goToEditPage(pet)}>
              Edit Pet
            </button>
            <button className={styles.button} onClick={() => deletePet(pet)}>
              Delete Pet
            </button>
          </div>
        )}
