...
const enquiryToResolved = async(enquiryID, adminEmail) => {
  ...
  return (
    <div>
        <button onClick={returnToAllEnquiries} className={styles.button}>Return</button>
        <div className={styles.infoWrapper}><span className={styles.label}>Date of Enquiry: </span>{viewEnquiry.date_created.slice(0, 10)}</div>
        <div className={styles.infoWrapper}><span className={styles.label}>Email: </span>{viewEnquiry.user_Email}</div>
        <div className={styles.infoWrapper}><span className={styles.label}>Full Name: </span>{viewEnquiry.first_name} {viewEnquiry.last_name}</div>
        <div className={styles.infoWrapper}><span className={styles.label}>Phone Number: </span>{viewEnquiry.phone}</div>

        <div className={styles.infoWrapper}><span className={styles.label}>Status: </span>
        <span className={viewEnquiry.status === "resolved" && styles.green || 
