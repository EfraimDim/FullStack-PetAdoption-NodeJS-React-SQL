...
function ViewUsers() {
  ...
  const { allPublicUsersArray, setAllPublicUsersArray, allAdminUsersArray, setAllAdminUsersArray, viewedUserDetails, tokenFromLocalforage, adminInfo, setLoadSpinner } = useContext(AppContext);
  ...
  const makeAdmin = async(user, adminEmail) =>{
    ...
    .then(async(makeAdmin) => {
      if (makeAdmin) {
        ...
        const makeUserAdmin = await axios.put(`http://localhost:5000/users/makeAdmin`, {
    ...
    {viewedUserDetails ? <ViewUserPets />  :<div>
          ...
          {allPublicUsersArray.map((user, index) => {  
                    return   (
                            <div className={styles.userHolder} key={index}>
                            <DisplayUser  user= {user}/>
                            <button className={styles.button} onClick={()=> makeAdmin(user, adminInfo.email)}>Make Admin</button>
                            </div>
                            )})}
            </div>
    </div>}
