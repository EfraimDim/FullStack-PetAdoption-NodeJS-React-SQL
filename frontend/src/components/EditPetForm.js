...
function EditPetForm() {
  const { setPetDetailsToEdit, petDetailsToEdit, petImages, tokenFromLocalforage, allPetsArray, setAllPetsArray, setLoadSpinner, adminInfo } =
      ...
      swal({
        ...
        if (willEdit) {
          ...
          if (selectedFile === null) {
            ...
            await axios.put(
              "http://localhost:5000/pets/editPetWithoutNewPhoto",
              {
                type: type,
            ...
            await axios.put("http://localhost:5000/pets/editPetWithNewPhoto", fd, { headers: headers });
            setLoadSpinner(false);
            setPetDetailsToEdit(null);
          }
  ...
  return (
    <div>
      ...
      <button className={styles.submit} onClick={returnToSearch}>
        Return
      </button>
      <br></br>
      <img className={styles.image} src={petImages[`${picturePath}`].default} alt="pet" />
      <form className={styles.form} onSubmit={editPet}>
        ...
        <select className={styles.input} required value={type} onChange={handleType}>
          <option defaultValue={petDetailsToEdit.type === "dog"} value="dog">
            Dog
          </option>
          <option defaultValue={petDetailsToEdit.type === "cat"} value="cat">
            Cat
          </option>
          <option defaultValue={petDetailsToEdit.type === "bird"} value="bird">
            Bird
          </option>
          <option defaultValue={petDetailsToEdit.type === "fish"} value="fish">
            Fish
          </option>
        </select>
        <InputLabel id="demo-simple-select-label">Status</InputLabel>
        <select className={styles.input} required value={adoptionStatus} onChange={handleAdoptionStatus}>
          <option defaultValue={petDetailsToEdit.adoption_status === "available"} value="available">
            Available
          </option>
          <option defaultValue={petDetailsToEdit.adoption_status === "adopted"} value="adopted">
            Adopted
          </option>
          <option defaultValue={petDetailsToEdit.adoption_status === "fostered"} value="fostered">
            Fostered
          </option>
        </select>
        <TextField
        ...
        <select className={styles.input} required value={hypoallergenic} onChange={handleHypoallergenic}>
          <option defaultValue={petDetailsToEdit.hypoallergenic === true} value={true}>
            Yes
          </option>
          <option defaultValue={petDetailsToEdit.hypoallergenic === false} value={false}>
            No
          </option>
        </select>
        <TextField
