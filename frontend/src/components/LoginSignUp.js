...
function ModalLoginSignUp() {
  ...
  return (
    <div>
      <Modal isOpen={modalIsOpen} onRequestClose={closeModal} style={customStyles} contentLabel="SignUp/Login Modal">
        <nav className={styles.navBar}>
          <div onClick={navSignUp} className={!isAdmin && isSignUp ? styles.white : styles.login}>
            Sign Up
          </div>
          <div onClick={navLogin} className={!isAdmin && !isSignUp ? styles.white : styles.login}>
            Login
          </div>
          <div onClick={navAdmin} className={isAdmin ? styles.white : styles.login}>
            Admin
          </div>
        </nav>
        {isAdmin ? (
          <>
            <form onSubmit={signUpAdmin} className={styles.form}>
              ...
              <TextField
                ...
                label="password"
              />
              <InputLabel sx={{ fontSize: "8px", height: "fit-content", width: "250px", textAlign: "center" }} id="demo-simple-select-label">
                *requires one capital letter, number, special char
              </InputLabel>
              <TextField
                size="small"
                required
                type="password"
                value={rePasswordAdmin}
                onChange={handleRepasswordAdmin}
                inputProps={{ minLength: 8, maxLength: 20 }}
                sx={inputStyles}
                label="re password"
              />
              ...
              <TextField
                ...
                onChange={handlePhoneNumberAdmin}
                inputProps={{ minLength: 12, maxLength: 12 }}
                sx={inputStyles}
                label="phone number"
              />
              <InputLabel sx={{ fontSize: "12px", height: "fit-content", width: "250px", textAlign: "center" }} id="demo-simple-select-label">
                *[0-9](3)-[0-9](3)-[0-9](4)
              </InputLabel>
        ...
        <button className={styles.close} onClick={(e) => closeModal(e)}>
          close
        </button>
      </Modal>
    </div>
