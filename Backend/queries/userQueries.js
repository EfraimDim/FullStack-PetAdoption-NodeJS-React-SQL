...
exports.signUpUserQuery = async(email, password, firstName, lastName, phoneNumber, admin, userID) => {
    try {
        const signUpUser = await query(SQL `INSERT INTO users (user_ID, email, password, first_name, last_name, phone, admin_status, bio) VALUES (${userID}, ${email.toLowerCase()}, ${password}, ${firstName}, ${lastName}, ${phoneNumber}, ${admin}, '')`)
        if(admin === true){
