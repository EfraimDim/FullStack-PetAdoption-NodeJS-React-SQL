...
exports.editPetWithNewPhoto = async (req, res) => {
  try {
    const { filename } = req.file;
    const { petID, type, adoptionStatus, name, colour, height, weight, bio, dietryRestrictions, hypoallergenic, breed, adminEmail, oldPicturePath } =
      req.body;
    const parseHeight = JSON.parse(height);
    const parseWeight = JSON.parse(weight);
    const parseHypoallergenic = JSON.parse(hypoallergenic);
    ...
    await petQueries.editPetWithNewPhotoQuery(
      filename,
      petID,
      type,
      adoptionStatus,
...
exports.editPetWithoutNewPhoto = async (req, res) => {
  try {
    ...
    await petQueries.editPetWithoutNewPhotoQuery(
      petID,
      type,
      adoptionStatus,
      name,
